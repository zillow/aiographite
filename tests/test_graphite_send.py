...
def test_generate_message_for_pickle():
	...
	expected_message = (b'\x00\x00\x00B\x80\x02]q\x00(X\x01\x00\x00\x00aq\x01'
...
def test_send_message_and_send():
	...
	async def test_send_multiple():
		...
		await aiographite.connect_to_graphite()	
		dataset = [('sproc%20performance.velo%40zillow%2Ecom.%3A%3AEH12', 3233, 1471640923),\
					('dit_400.zpid%40zillow%2Ecom.EHT%3A%3Adisk_usage_per_host', 2343, 1471640976)]
		...
		writer.close()
		aiographite.disconnect()
		message = (b"\x00\x00\x00\x9c\x80\x02]q\x00(X2\x00\x00\x00sproc"
			...
			b"%2Ecom.EHT%3A%3Adisk_usage_per_hostq\x04J\x90u\xb7WM'\t\x86q\x05\x86q\x06e.")
		assert message == data
